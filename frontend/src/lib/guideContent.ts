...
export const GUIDE_SECTIONS: GuideSection[] = [
  ...
  // ── Section 3: 매도/청산 규칙 ──
  {
    ...
    content: {
      paragraphs: [
        '수익을 지키고 손실을 최소화하기 위해, 청산은 하드 룰(손절 → BB 상단 익절 → 트레일링) 우선 + 스코어링 보조 구조로 동작합니다.',
        '동적 ATR 손절: 고정 -3% 대신 시장 레짐별 배수를 적용합니다. 횡보 2.8배, 추세 2.2배, 변동성 폭발 2.5배로 즉시 전량 손절합니다.',
        'BB 상단 도달 익절: 가격이 볼린저 밴드 상단 이상이고 최소 수익 마진(0.45%)을 충족하면, 스코어링·품질 게이트를 거치지 않고 즉시 매도합니다. 1차 익절 전이면 40% 매도, 이후면 잔량 전량 매도입니다.',
        '1차 익절(스코어링 기반, 기본 40%): 스코어가 실효 임계치를 넘더라도, 적응형 최소 수익률을 충족하고 품질 게이트(가격 ≥ BB 중앙선 또는 충분한 초과수익)를 통과해야 실행됩니다.',
        '1차 익절 후에는 스코어링 매도를 비활성화하고 하드 룰(손절 + BB 상단 익절 + 트레일링)만 사용합니다. 잔량은 BB 상단 도달 또는 트레일링 조건(max(고점-ATR×적응배수, 진입가×(1+min_profit_margin)))에서 전량 정리됩니다.',
      ],
      tables: [
        {
          title: '청산 파이프라인',
          ...
          data: [
            { key: '1', stage: '동적 손절', condition: '가격 ≤ 매수가 - ATR×레짐배수 (횡보2.8/추세2.2/변동2.5)', action: '전량 즉시 매도' },
            { key: '2', stage: 'BB 상단 익절', condition: '가격 ≥ BB 상단 AND 수익 ≥ 최소마진(0.45%)', action: '미반매도→40% 매도 / 반매도→전량 매도' },
            { key: '3', stage: '1차 익절 후보 (스코어링)', condition: 'exit_score ≥ 실효 임계치 + 적응형 최소수익률 충족 + 품질 게이트 통과', action: '보유 수량의 40% 매도' },
            { key: '4', stage: '트레일링 대기', condition: '1차 익절 이후', action: '스코어링 비활성, 하드 룰만 모니터링' },
            { key: '5', stage: '2차 익절(잔량)', condition: 'BB 상단 도달 또는 가격 ≤ max(고점-ATR×적응배수, 진입가×(1+min_profit_margin))', action: '남은 전량 매도' },
          ],
        },
      ],
      highlights: [
        '핵심은 BB 상단 도달 시 즉시 익절(하드 룰) + 조기 익절 억제(스코어링 품질 게이트) + 수익 잠금 강화(트레일링)입니다. 세 가지를 결합해 손익비를 개선합니다.',
      ],
    },
  },
