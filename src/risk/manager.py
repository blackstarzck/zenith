...
class RiskManager:
    ...
    def calc_position_size(self, current_balance: float,
        ...
        고정비율(max_position_ratio) 기반 사이징.
        Kelly 공식은 현재 비활성화 — 거래 이력 축적 후 재활성화 예정.

        Args:
            current_balance: 현재 가용 잔고 (KRW)
            recent_sell_trades: (미사용) 향후 Kelly 재활성화 시 사용.
        ...
        logger.info('[포지션 사이징] 고정비율 %.0f%% → %.0f KRW',
                    self._params.max_position_ratio * 100, fixed_size)
        return fixed_size
